...
class DF:
    ...
    def to_csvfile(self, output_file):
    ...
    #update the header of the dataframe
    ...
    def rename_columns(self, columns = None, inplace=False):
        if inplace :
            ...
            new_frame = DF(self.data, self.columns)
    ...
    def filter(self, colname, colvalue):
        ...
        for ridx, row in enumerate(self.data):
            if row[cidx] == colvalue:
                result.append(ridx)
        return DF([self.data[idx] for idx in result], self.columns)
